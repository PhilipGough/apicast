  ...
  describe('.content', function()
    describe('when there is a new upstream in the context', function()
      it('changes the upstream', function()
        ...
        stub.new(upstream, 'rewrite')
        stub.new(upstream, 'call')

        local ctx = { [policy] = upstream }
        policy:content(ctx)

        assert.spy(upstream.rewrite).was_called_with(upstream)
  ...
  describe('.balancer', function()
    ...
    describe('when the upstream has not been changed in previous phases', function()
      it('does not call the balancer', function()
        stub(apicast_balancer, 'call')
        local policy = UpstreamPolicy.new()

        for _, val in ipairs({ false, nil }) do
          policy:balancer({ upstream_changed = val })

          assert.stub(apicast_balancer.call).was_not_called()
        end
      end)
    end)
  end)
